...
const rocketModelMatrix = new glance.TimeSensitive(
    (time) =>
        mat4.multiply(
            // mat4.fromRotation(rocketId * (Math.PI / rocketCount), [0, 1, 0]),
            mat4.identity(),
            mat4.multiply(
                mat4.multiply(
                    // mat4.fromRotation(rocketId + rocketSpeed * time, [0, 0, -1]),
                    mat4.fromRotation(rocketId * (Math.PI * 2 / rocketCount) + rocketSpeed * time, [0, 0, -1]),
...
const rocketDrawCall = glance.createDrawCall(
    ...
    {
        uniforms: {
            u_modelMatrix: (time) => rocketModelMatrix.getAt(time),
            u_normalMatrix: (time) => mat3.fromMat4(mat4.transpose(mat4.invert(rocketModelMatrix.getAt(time)))),
            u_viewMatrix: () => mat4.invert(viewMatrix.get()),
            u_viewPos: () => vec3.transformMat4(vec3.zero(), viewMatrix.get()),
        },
        textures: [
