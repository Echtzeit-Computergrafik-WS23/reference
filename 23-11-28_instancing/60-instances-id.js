...
const instancedPhongVertexShader = `#version 300 es
    ...
    const float rocketScale = 0.05;
    const float rocketDistance = -1.5;
    const float rocketSpeed = 0.0003;
    ...
    void main() {
        mat4 modelMatrix = rotation(vec3(0.0, -1.0, 0.0), float(gl_InstanceID) * (pi / ${rocketCount}.0)) *
            (
                (
                    rotation(vec3(0.0, 0.0, 1.0), float(gl_InstanceID) + rocketSpeed * u_time)
                    *
                    translation(vec3(rocketDistance, 0.0, 0.0))
                ) * scale(vec3(rocketScale))
            );
        mat4 normalMatrix4 = transpose(inverse(modelMatrix));
        ...
        f_worldPos = vec3(modelMatrix * vec4(a_pos, 1.0));
        f_normal = normalMatrix * a_normal;
        f_texCoord = a_texCoord;
