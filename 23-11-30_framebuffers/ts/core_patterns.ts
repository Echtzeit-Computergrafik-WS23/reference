 ...
 * @param options Additional texture options:
 * - `flipY`: Whether to flip the images vertically, defaults to `true`.
...
function getFramebufferSize(framebuffer: Framebuffer): [number, number]
{
    if (framebuffer.color.length > 0) {
        return [framebuffer.color[0].width, framebuffer.color[0].height];
    } else if (framebuffer.depth !== null) {
        return [framebuffer.depth.width, framebuffer.depth.height];
    } else if (framebuffer.stencil !== null) {
        return [framebuffer.stencil.width, framebuffer.stencil.height];
    } else {
...
class FramebufferStack
{
    ...
    private _stack: Array<Framebuffer> = [];
     ...
     */
    public push(gl: WebGL2, framebuffer: Framebuffer): void
    {
