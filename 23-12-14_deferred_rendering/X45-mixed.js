...
const whiteSolidVertexShader = `#version 300 es
    precision highp float;
    ...
    void main() {
        gl_Position = u_viewProjection * u_viewXform * u_modelMatrix * vec4(a_pos, 1.0);
    }
`;

const whiteSolidFragmentShader = `#version 300 es
    precision mediump float;
...
const deferredVAO = glance.createVAO(
    ...
    "deferred-vao",
    deferredIBO,
    glance.buildAttributeMap(deferredShader, [deferredABO]),
);

// White Solid Geo -------------------------------------------------------------

const whiteSolidShader = glance.buildShaderProgram(gl, "whiteSolid-shader", whiteSolidVertexShader, whiteSolidFragmentShader, {
    u_modelMatrix: mat4.identity(),
    u_viewProjection: viewProjection,
});

const whiteSolidIBO = glance.createIndexBuffer(gl, glance.createSphereIndices(16, 32));

const whiteSolidABO = glance.createAttributeBuffer(gl, "whiteSolid-abo", glance.createSphereAttributes(sphereRadius, 16, 32, {
    normals: false,
    uvs: false
}), {
    a_pos: { size: 3, type: gl.FLOAT },
...
const whiteSolidVAO = glance.createVAO(
    gl,
    "whiteSolid-vao",
    whiteSolidIBO,
    glance.buildAttributeMap(whiteSolidShader, whiteSolidABO),
);
...
const whiteSolidDrawCall = glance.createDrawCall(
    gl,
    whiteSolidShader,
    whiteSolidVAO,
    {
...
setRenderLoop((time) =>
{
    ...
    {
        gl.blitFramebuffer(0, 0, 512, 512, 0, 0, 512, 512, gl.DEPTH_BUFFER_BIT, gl.NEAREST);
    }
    framebufferStack.pop(gl);

    glance.performDrawCall(gl, deferredDrawCall, time);
    glance.performDrawCall(gl, whiteSolidDrawCall, time);
});
