...
const toneMappingVertexShader = `#version 300 es
    precision highp float;
...
const toneMappingFragmentShader = `#version 300 es
    ...
    void main()
    {
        ...
        o_fragColor = vec4(gammaCorrected, 1.0);
    }
`;
...
const toneMappingShader = glance.buildShaderProgram(gl, "toneMapping-shader", toneMappingVertexShader, toneMappingFragmentShader, {
    u_texture: 0,
});

const toneMappingIBO = glance.createIndexBuffer(gl, glance.createQuadIndices());

const toneMappingABO = glance.createAttributeBuffer(gl, "toneMapping-abo", glance.createQuadAttributes({
    uvs: false,
}), {
    a_pos: { size: 2, type: gl.FLOAT },
});

const toneMappingVAO = glance.createVAO(
    gl,
    "tonemapping-vao",
    toneMappingIBO,
    glance.buildAttributeMap(deferredShader, [toneMappingABO]),
);
...
const toneMappingTexture = glance.createTexture(gl, "toneMapping-texture", 512, 512, gl.TEXTURE_2D, null, {
    useAnisotropy: false,
    internalFormat: gl.RGBA16F,
    levels: 1,
    filter: gl.NEAREST,
...
const toneMappingFrameBuffer = glance.createFramebuffer(gl, "toneMapping-framebuffer", toneMappingTexture);
...
const toneMappingDrawCall = glance.createDrawCall(
    gl,
    toneMappingShader,
    toneMappingVAO,
    {
        textures: [
            [0, toneMappingTexture],
        ],
        cullFace: gl.NONE,
        depthTest: gl.NONE,
    }
...
setRenderLoop((time) =>
{
    ...
    framebufferStack.push(gl, toneMappingFrameBuffer);
    {
    ...
    gl.clear(gl.COLOR_BUFFER_BIT);
    glance.performDrawCall(gl, toneMappingDrawCall, time);
});
