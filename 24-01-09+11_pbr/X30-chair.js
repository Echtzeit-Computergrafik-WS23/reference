...
const shaderProgram = glance.buildShaderProgram(gl,
    ...
    {
        u_modelMatrix: mat4.fromTranslation([0, -.65, 0]),
...
// Model by Fernando Quinn, licensed under CC0 from https://polyhaven.com/a/BarberShopChair_01
const { attributes: chairAttr, indices: chairIdx } = await glance.loadObj("./obj/barbershop_chair.obj", {
    tangents: true
});

const objectIBO = glance.createIndexBuffer(gl, chairIdx);

const objectABO = glance.createAttributeBuffer(gl, "object-abo", chairAttr, {
...
// Textures by Fernando Quinn, licensed under CC0 from https://polyhaven.com/a/BarberShopChair_01
const texAlbedo = await glance.loadTextureNow(gl, "./img/barbershop_chair_albedo.avif");
const texNormal = await glance.loadTextureNow(gl, "./img/barbershop_chair_normal.avif");
const texRoughness = await glance.loadTextureNow(gl, "./img/barbershop_chair_roughness.avif");
const texMetalness = await glance.loadTextureNow(gl, "./img/barbershop_chair_metallic.avif");
